...
router.delete('/:id', authJwt, (req, res, next) => {
  ...
  Comment.findById(id).exec().then(comment => {
    ...
    comment.remove().then(() => {
      Post.findById(comment.post).exec().then(post => {
        post.comments = removeEl(comment._id)
        post.save()
        User.findById(comment.user).exec().then(user => {
          user.comments = removeEl(comment._id)
          user.save()
